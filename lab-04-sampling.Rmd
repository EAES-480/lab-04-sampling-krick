...
author: "Rachel Krick: EAES 480 — Modern Statistics in Earth & Environmental Science"
...
> I chose `gpp` or Gross Primary Productivity because it measures the amount of carbon that plants uptake through photosynthesis and the seasonality/day-night pattern of photosynthesis would be interesting to analyze. I expect that during the daytime there will be higher gpp values and lower at night. During the spring, I would expect gpp values to be increasing and to peak in the summer. Then during the fall, I would expect gpp to be decreasing and to be at its lowest in the winter. 
...
|----------|-------|-------------|----------------------------|
|  fc      | µmolCO2 m-2 s-1 | Carbon Dioxide (CO2) turbulent flux |  increase in summer, decrease in winter |
|  le      | W m-2 | Latent heat turbulent flux | increase in summer/decrease in winter  |
|  ta      | deg C |  Air temperature |  increase in summer/day, decrease in winter/night |
| reco     | µmolCO2 m-2 s-1 |  Ecosystem Respiration |   increase in summer, decrease in winter |
| gpp      |  µmolCO2 m-2 s-1   | Gross Primary Productivity  | increase in summer/day, decrease in winter/night |
...
> Overall, during the fall and winter `gpp`appears to be decreasing and at its lowest, whearas during the spring and summer `gpp` appears to be increasing and at its peak. A daily cycle might also be reflected by the variance from positive and negative values between each month. There are a few outliers, such as an extremely negative `gpp` value in April.
...
ggplot(df, aes(x = gpp)) +
  geom_histogram(bins = 50, alpha = 0.7, color="black",fill="darkgreen") +
  theme_classic(base_size = 18) +
  labs(x = response_var, y = "Count", title = "Population distribution (histogram)")

ggplot(df, aes(x = gpp)) +
  geom_density(alpha = 0.7,color="black", fill="darkgreen") +
  theme_classic(base_size = 18) +
...
> The distribution is slightly right skewed and is unimodal. There are a large number of values centered around zero, with the mean probaly being slightly above zero. Due to the skew, the median is probably lower than the mean. The spread is fairly wide with values ranging from ~-25 to 45.
...
> Both month and day/night appear to explain the variability in my response (as I had predicted). However, night and day appears to explain more variability overall. This is because the separation or variance among the values between night and day is much sharper than the monthly variation, which appears to be more gradual. 
...
> My one-sample estimate is fairly close to the population mean/SD. As the difference is small, I am not very surprised by the difference.
...
ggplot(tibble(mean_est = means), aes(mean_est)) +
  geom_histogram(bins = 40, alpha = 0.8, fill="darkgreen") +
...
> The sampling distribtion does appear to be centered (or pretty close to being centered) on the population mean. If I increase `n_samp` the sampling distribution becomes closer to or approaches the normal distribution. 
...
> The SRS strategy better aproximated the population mean and SD for my response variable. Stratifcation by month may not have helped here because while the variation in `gpp` is affected by seasonal cycles, it is also greatly affected by cycles of day and night. While the seasonal changes that occur duriung different months do matter when it comes to `gpp`, ignoring the day/night cycle causes poorer approximations of the population. 
...
> Seasonality matters for sampling because the population is not uniform overtime (in the case of `gpp` values are lower in winter and higher in summer). If you sample disproportionately from one season you might risk creating a biased estimate of the mean or under/overestimating variability. If latent grouping variables are ignored sampe estimates may be biased or inaccurate and important patterns may be misinterpreted or missed completely. In EAES field studies, stratification is essential, when the groups differ dramatically (such as) or environmental gradients (such as elevation or soil type) are strong. One trade-off of stratified sampling is convenience in the collections of samples in the field, as you would need prior knowledge of what would make good strata and you would have to visit more locations in order to collect samples. 
...
> Systematic sampling approximated the population mean worse than SRS. The daily cycle might be interacting with my chosen `k`. Certain k values may have repeatedly sampled similar times of day, such as just the highest or lowest values leading to over- or under-representation of `gpp`. In the case of my chose `k` of 24, it was an overestimate.
...
> Cluster sampling may have higher variance than SRS because `gpp` observations within a cluster are autocorrelated with the time of day. Clusters tend to be similar to one another and estimates from cluster sampling may overrepresent a specific instance in the cycle. In the case of `gpp` clustering by days might not capture the full daily cycle, with some samples overrepresenting daytime `gpp` and others nighttime `gpp`. As a result, different sampled clusters may produce very different means depending on when they occur, increasing variability across samples.
...
df %>%
  ...
  geom_line(alpha = 0.25, color="darkgreen") +
  theme_classic(base_size = 18) +
...
> The quasi-continuous sampling clearly preserves the daily pattern in `gpp`, as shown by the repeated 24-hour peaks and troughs in the July time series. While this subset primarily highlights the daily pattern, I would assume a quasi-continuous sampling of the full year would also preserve the seasonal pattern, as measurements are taken very frequently. In an EAES context, quasi-continuous sampling is preferable  to sparse discrete sampling when studying dynamic processes that have shorter term variability, like the day and night cycle of photosynthesis.
...
> The quasi-continuous strategy gave the best estimate of the population mean and standard deviation. This is because it preserved both the daily and seasonal patterns of `gpp`. The other strategies did not work as well either because they only captured specific parts of the daily cycle (such as systematic) or because they emphasized monthly or seasonal cycle over daily (such as stratification by month). In a real EAES study with limited field days, I would propose a hybrid strategy combining seasonal stratification with clustered full-day sampling. This would capture both the seasonal and diurnal variability that impacts `gpp`.
